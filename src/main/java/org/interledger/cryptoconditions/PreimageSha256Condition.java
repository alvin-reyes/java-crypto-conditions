 ...
 * condition hash.
 */
public interface PreimageSha256Condition extends SimpleCondition {

  /**
   * Required-args Constructor.  Constructs an instance of {@link PreimageSha256Condition} based on
   * a supplied preimage.
   *
   * @param preimage An instance of byte array containing preimage data.
   *
   * @deprecated This constructor will be made package-private in future versions because it is only
   *     used from within {@link PreimageSha256Fulfillment}.
   *
   ...
   *     PreimageSha256Fulfillment#getCondition()}.
   */
  @Deprecated
  static PreimageSha256Condition of(final byte[] preimage) {
    return new Impl(preimage);
  }

  /**
   * Constructs an instance of {@link PreimageSha256Condition} using a fingerprint and cost. Note
   * that this constructor variant does not include a preimage, and is thus intended to be used to
   * construct a condition that does not include a preimage (for example, if a condition is supplied
   * by a remote system).
   *
   ...
   */
  static PreimageSha256Condition of(final long cost, final byte[] fingerprint) {
    return new Impl(cost, fingerprint);
  }

  /**
   * An extension of {@link Sha256Condition} that implements {@link PreimageSha256Condition}.
   */
  final class Impl extends Sha256Condition implements PreimageSha256Condition {
     ...
     * @param preimage An instance of byte array containing preimage data.
     *
     * @deprecated This constructor will be made package-private in future versions because it is
     *     only used from within {@link PreimageSha256Fulfillment}.
     *
     *     <p>Developers that wish to create a new {@link PreimageSha256Condition} from the preimage
     *     should instead create a new {@link PreimageSha256Fulfillment} and call {@link
     *     PreimageSha256Fulfillment#getCondition()}.
     */
    @Deprecated
    private Impl(final byte[] preimage) {
      super(
     ...
     * Constructs an instance of {@link PreimageSha256Condition} using a fingerprint and cost. Note
     * that this constructor variant does not include a preimage, and is thus intended to be used to
     * construct a condition that does not include a preimage (for example, if a condition is
     * supplied by a remote system).
     *
     * @param cost        The cost associated with this condition.
     * @param fingerprint An instance of byte array that contains the calculated fingerprint for
     */
    private Impl(final long cost, final byte[] fingerprint) {
